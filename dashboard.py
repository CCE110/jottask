...
def admin_create_opensolar_for_task(task_id):
    ...
    # 1. Create OpenSolar project. The body can supply a pre-existing
    # OpenSolar URL to skip creation (useful when OPENSOLAR_EMAIL /
    # OPENSOLAR_PASSWORD aren't on this Railway service so the connector
    # can't auth here — we create the project from elsewhere and post
    # only the URL through this endpoint to finish the wiring).
    pre_made_url = (body.get('os_url') or '').strip()
    if pre_made_url:
        project_id = pre_made_url.rsplit('/projects/', 1)[-1].split('/', 1)[0] or None
        os_url = pre_made_url
    else:
        # make_opensolar returns (pid, url); capture stdout so we can surface
        # the underlying error if it returns (None, None).
