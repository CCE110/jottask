...
class AIEmailProcessor:
    ...
    def _load_processed_emails(self):
        """Load set of already-processed email Message-IDs AND UIDs from Supabase"""
        try:
            # Try loading both message_id and uid
            try:
                result = self.tm.supabase.table('processed_emails') \
                    .select('message_id,uid') \
                    .execute()
            except Exception:
                # uid column might not exist — fall back to message_id only
                print("⚠️ uid column not found, loading message_id only")
                result = self.tm.supabase.table('processed_emails') \
                    .select('message_id') \
                    .execute()
            ids = set()
    ...
    def _mark_email_processed(self, message_id, uid_str=''):
        ...
        try:
            ...
            err_msg = str(e).lower()
            if 'uid' in err_msg or 'column' in err_msg or '400' in err_msg or 'schema' in err_msg:
                # uid column likely doesn't exist — retry without it
                print(f"⚠️ uid column issue, retrying without uid: {e}")
                try:
                    self.tm.supabase.table('processed_emails').insert({
                        'message_id': message_id,
                        'processed_at': datetime.now().isoformat(),
                    ...
                    if 'duplicate' in str(e2).lower() or '409' in str(e2) or '23505' in str(e2):
                        # Already exists — that's fine
                        self.processed_emails.add(message_id)
                        if uid_str:
                            self.processed_emails.add(uid_str)
                        return
                    print(f"❌ Failed to mark email processed (retry): {e2}")
            elif 'duplicate' in err_msg or '409' in err_msg or '23505' in err_msg:
                # Duplicate key — already processed, still add to in-memory set
                self.processed_emails.add(message_id)
                if uid_str:
                    self.processed_emails.add(uid_str)
                return
